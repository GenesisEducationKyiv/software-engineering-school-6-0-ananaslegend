...
func (r *Repository) CreateSubscription(ctx context.Context, p domain.CreateSubscriptionParams) (*domain.Subscription, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("subscription.Repository.CreateSubscription: %w", err)
	}
	return &sub, nil
}

func (r *Repository) CreateConfirmation(ctx context.Context, subscriptionID int64) error {
	_, err := r.conn(ctx).Exec(ctx, `
		INSERT INTO confirmation_notifications (subscription_id) VALUES ($1)
	`, subscriptionID)
	if err != nil {
		return fmt.Errorf("subscription.Repository.QueueConfirmation: %w", err)
	}
	return nil
}
...
func (r *Repository) MarkConfirmed(ctx context.Context, p domain.MarkConfirmedParams) error {
	...
	`, p.Now, p.ID)
	if err != nil {
		return fmt.Errorf("subscription.Repository.MarkConfirmed: %w", err)
	}
	return nil
}
...
func (r *Repository) DeleteByUnsubscribeToken(ctx context.Context, token string) (bool, error) {
	...
	`, token)
	if err != nil {
		return false, fmt.Errorf("subscription.Repository.DeleteByUnsubscribeToken: %w", err)
	}
...
func (r *Repository) ListByEmail(ctx context.Context, email string) ([]domain.SubscriptionView, error) {
	...
	`, email)
	if err != nil {
		return nil, fmt.Errorf("subscription.Repository.ListByEmail: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v.ID, &v.RepoOwner, &v.RepoName, &v.ConfirmedAt, &v.CreatedAt); err != nil {
			return nil, fmt.Errorf("subscription.Repository.ListByEmail: scan: %w", err)
		}
		result = append(result, v)
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("subscription.Repository.ListByEmail: rows.Err: %w", err)
	}
	return result, nil
}
